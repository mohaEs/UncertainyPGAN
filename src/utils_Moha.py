...
def softdiceloss(inputs, targets, smooth=1):
...
def train_i2i_UNet3headGAN(
    ...
    ####
    list_epochs = [num_epochs] #[50, 50, 150]
    list_lambda1 = [1] #[1, 0.5, 0.1]
    list_lambda2 = [0.001] #[0.0001, 0.001, 0.01]
    for num_epochs, lam1, lam2 in zip(list_epochs, list_lambda1, list_lambda2):
        for eph in range(num_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                #first gen
                netD_A.eval()
                total_loss = lam1*F.l1_loss(rec_B, xB) + lam2*bayeGen_loss(rec_B, rec_alpha_B, rec_beta_B, xB) + dice_weight * softdiceloss(rec_B, xB)
                #print(total_loss)
                t0 = netD_A(rec_B)
